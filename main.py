...
def find_all_for_teach(path, all_letters, let):
...
def teach_prez_neiro_new():
    ...
    for let in s_big:
        path = path_big + "\\" + let + "\\"
        find_all_for_teach(path, all_letters, let)
    for let in s_let.upper():
        if let not in s_big:
            let2 = let.lower()
            path = path_small + "\\" + let2 + "\\"
            find_all_for_teach(path, all_letters, let2)
        else:
            let2 = let.lower()
            path = path_big + "\\" + let + "\\"
            find_all_for_teach(path, all_letters, let2)
...
if __name__ == '__main__':
    ...
    #teach_prez_neiro_new()
    img = rt.RetouchNSplitImg("img/test3.jpg")
    size = img.test_img()
